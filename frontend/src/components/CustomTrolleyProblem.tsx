...
function getEvenlySpacedPointsCentered(
  path: SVGPathElement,
  count: number,
  paddingPercent: number = 0.2 // 5% padding on each end
): { x: number; y: number }[] {
  ...
  const start = length * paddingPercent
  const end = length * (1 - paddingPercent)

  // If count is 2, simulate spacing for 5 but select only 2 points (the 1st and the 2nd)
  if (count === 2) {
    const simulatedCount = 5
    const usableLength = end - start
    const spacing = usableLength / (simulatedCount - 1)

    const point1 = path.getPointAtLength(start + (spacing * 1))
    const point2 = path.getPointAtLength(start + (spacing * 2))
  ...
  for (let i = 0; i < count; i++) {
    const point = path.getPointAtLength(start + (usableLength * i) / (count - 1))
...
export default function CustomTrolleyProblem({ sectionLabel="The Trolley Problem", restore=null, numberOfTopTrackVictims=1, numberOfBottomTrackVictims=5, scenarioName = "trolley" }: CustomTrolleyProblemProps) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/last_decision/?scenario_name=${scenarioName}&session_id=${sessionId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.response_id) {
  ...
  const handleReset = () => {
    ...
    gsap.set(topVictims, { opacity: 1 });
    gsap.set(bottomVictims, { opacity: 1 });
    ...
    <div className="max-w-3xl w-full text-center">
      ...
      <div className="mt-4">
        ...
        <svg viewBox="0 0 800 400" className="w-full h-[400px]">
            ...
            <path d="M291.844 257.986C287.869 265.936 293.86 250.118 296.103 246.914C303.514 236.326 312.369 227.322 321.655 218.38C347.988 193.023 381.515 172.758 413.218 154.925C484.824 114.646 566.534 89.8549 647.875 80.3966C677.941 76.9006 708.35 76.1378 738.587 76.1378C741.122 76.1378 808.459 73.3099 800 74.5183" fill="none" opacity="1" stroke="#000000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="7.11036"/>
            <path d="M16.3033 111.911C12.1663 118.117 55.9877 129.931 59.7425 130.65C110.182 140.308 159.878 156.105 210.502 164.294C267.157 173.459 320.583 192.33 376.167 205.604C444.244 221.861 512.714 236.737 579.735 257.134C625.532 271.073 672.701 282.874 719.422 293.334C734.077 296.615 749.149 297.784 763.713 301.425C767.254 302.311 803.355 306.407 800 304.969" fill="none" opacity="1" stroke="#000000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="7.11036"/>
            <path d="M-2.0093 181.329C-6.17571 180.287 22.8635 186.034 28.2278 186.439C48.7684 187.99 63.9007 197.995 83.1656 203.048C139.098 217.719 197 225.628 252.664 242.229C282.445 251.111 313.201 255.969 342.949 264.8C482.22 306.146 657.795 344.623 800.735 368.446" fill="none" opacity="1" stroke="#000000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="7.11036"/>
            <path d="M363.817 275.873C358.801 277.545 395.475 250.239 401.294 246.914C429.634 230.719 460.63 216.536 490.302 203.048C559.495 171.597 635.731 152.91 711.331 146.407C733.431 144.506 791.137 143.468 800.735 141.834" fill="none" opacity="1" stroke="#000000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="7.11036"/>
            <g opacity="0">
            ...
            <g clipPath="url(#ArtboardFrame)" id="victim-paths">
            <path id="default-victims-path" d="M412.357 199.011C409.073 199.011 423.499 192.204 426.69 190.609C436.584 185.662 445.635 179.699 455.852 175.287C481.381 164.263 506.644 153.931 533.451 146.62C554.312 140.93 574.137 131.257 595.234 125.861C631.133 116.677 669.285 109.699 705.949 104.607C725.766 101.855 746.707 99.6841 766.743 99.1703C776.065 98.9313 786.344 101.437 795.41 99.1703" fill="none" opacity="0" stroke="#007aff" strokeLinecap="round" strokeLinejoin="round" strokeWidth="10"/>
            <path id="alternate-victims-path" d="M436.081 264.748C433.616 264.748 452.714 267.023 454.369 267.22C462.305 268.165 470.173 269.613 478.094 270.68C505.678 274.393 532.819 280.969 560.141 286.002C637.3 300.215 713.503 321.871 789.479 340.865" fill="none" opacity="0" stroke="#007aff" strokeLinecap="round" strokeLinejoin="round" strokeWidth="10"/>
            </g>
